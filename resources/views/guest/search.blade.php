        ...
        <section class="search__results">
            ...
            <div class="searchbar">
                <div class="large-container">
                    <div class="main-searchbar">
                        ...
                        <div class="range" v-cloak>
                            <input type="range" min="1000" max="100000" v-model="radius">
                            <span>@{{Number(radius / 1000).toFixed(1)}} Km</span>
                        </div>
                        {{-- Dropdown list --}}
                        <ul v-if="dropdownResults && query != ''" :class="dropdownResults == true ? 'advanced-dropdown' : '-hide'" v-cloak>
            ...
            <div class="filters">
                <div class="large-container">
                    <div>
                        ...
                        <select v-on:click="orderByPrice" v-model="priceOrder">
                            <optgroup label="Prezzo">
                                <option selected disabled value="">Qualsiasi</option>
                    ...
                    <div>
                        <label for="num_bed">Letti</label>
                        <select v-on:click="numBed" v-model="selectedNumBed" v-cloak>
                            <optgroup label="Numero letti">
                                <option v-for="num in numBedList" v-bind:value="num" v-cloak>@{{ num }}</option>
                            </optgroup>
                        </select>
                    </div>
                            ...
                            <div id="checkboxes">
                                <template v-for="service in servicesNameArray">
                                    <div class="template-container">
                                        <input :id="service" type="checkbox" :value="service" v-model="checked.service" v-cloak>
                                        <label :for="service" v-cloak>@{{ service }}</label>
                                    </div>
                                </template>
                            </div>
                        </div>
                    ...
                    <div>
                        <label for="numMq">Metratura</label>
                        <select v-on:click="numMq" v-model="selectedMq">
                            <optgroup label="Metri quadri">
                                <option v-for="num in numMqList" v-bind:value="num" v-cloak>@{{ num }}</option>
                            </optgroup>
                        </select>
                    </div>
                </div>
            ...
            <div class="results">
                <div class="large-container">
                    <h3 v-if="isQueryActive" v-cloak>Results for "@{{lastQuery}}"</h3>
                    <ul v-if="myApartmentsResults.length != []">
                        <li v-for="apartment in myApartmentsResults" v-show="visible(apartment.services) && ((selectedNumBed == 'Qualsiasi' && selectedMq == 'Qualsiasi') || ((apartment.num_bed == selectedNumBed) && (selectedNumBed != 'Qualsiasi' && selectedMq == 'Qualsiasi')) || ((apartment.mq == selectedMq) && (selectedMq != 'Qualsiasi' && selectedNumBed == 'Qualsiasi')) || ((apartment.mq == selectedMq && apartment.num_bed == selectedNumBed) && (selectedMq != 'Qualsiasi' && selectedNumBed != 'Qualsiasi')))" class="card-apartment">
                            <div class="card-apartment__info">
                                ...
                                <div class="text">
                                    <div class="text__info">
                                        ...
                                        <span>4,91 (147)</span>
                                        <div v-cloak><h6>@{{apartment.name}}</h6></div>
                                        <div v-cloak><p>@{{apartment.street}}, @{{apartment.city}}, @{{apartment.region}}</p></div>
                                    </div> 
                                    <ul class="text__services">
                                        <li v-cloak>@{{apartment.num_bed}} bed(s)</li>
                                        <li v-cloak>@{{apartment.mq}}m&#178;</li>
                                    </ul>
                                </div>
                            </div>
                    ...
                    <div v-else-if="isQueryActive == true || checked.service == []">
